...
type ProviderCoverage struct {
	...
	MissingPrices int     `json:"requests_without_price"`
	MissingTokens int     `json:"requests_without_tokens"`
	KnownCost     float64 `json:"known_cost_usd"`
}

type DeliveryReport struct {
	Providers          map[string]*ProviderCoverage `json:"providers"`
...
func deliveryReport(root string, paths []string) (DeliveryReport, error) {
	d := DeliveryReport{Providers: map[string]*ProviderCoverage{}, Elapsed: map[string]float64{}, Notes: []string{"Known cost is a lower bound; missing coordinator/reviewer prices prevent a total-dollar verdict.", "Study preparation and tooling are reported separately without amortization. CI wait is unmeasured. Historical reviewed acceptance is distinct from verified landing."}}
	...
	for _, p := range paths {
		...
		if err := readJSON(filepath.Join(dir, "provider-ledger.json"), &ledger); err != nil {
			...
			incomplete := !ledger.Complete || len(ledger.Generations) == 0
			for _, r := range ledger.Generations {
				if r.Cost == nil || r.Prompt == nil || r.Output == nil {
					incomplete = true
				}
			}
			if incomplete {
			...
			for _, r := range ledger.Generations {
				...
				seen[key] = r
				coverage := d.Providers[r.Provider]
				if coverage == nil {
					coverage = &ProviderCoverage{}
					d.Providers[r.Provider] = coverage
				}
				coverage.Requests++
				if r.Cost == nil {
					coverage.MissingPrices++
				} else {
					coverage.KnownCost += *r.Cost
				}
				if r.Prompt == nil || r.Output == nil {
					coverage.MissingTokens++
				}
				if r.Cost != nil {
					if *r.Cost < 0 {
